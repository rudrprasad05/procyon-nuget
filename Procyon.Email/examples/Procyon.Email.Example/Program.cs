...
static EmailMessage BuildMessage(IEmailSender emailSender, ComposeRequest request)
{
    ...
    var builder = emailSender.Create()
        ...
        .Header("X-Procyon-Example", "email-compose")
        .Tag("source", "procyon-email-example")
        .Tag("priority", request.Priority.ToString().ToLowerInvariant())
        .Metadata("example", "Procyon.Email.Example")
...
static string CreateHtmlBody(ComposeRequest request)
{
    ...
    return $$"""
        ...
        <body style="font-family: Arial, sans-serif; color: #17202a; line-height: 1.5;">
          <p>{{greeting}}</p>
          <p>{{body}}</p>
          <p>Regards,<br>Procyon Email Example</p>
        </body>
        </html>
        """;
}
...
static string RenderPage(ComposeRequest? values = null, SendFeedback? feedback = null)
{
    ...
    return $$"""
        ...
        <head>
          ...
          <title>Procyon.Email Example</title>
          <style>
            :root {
              color-scheme: light;
              --bg: #f6f7f9;
          ...
          <header>
            <div class="shell topbar">
              <div class="brand">
                ...
                <div>
                  <h1>Procyon.Email Example</h1>
                  <p class="subtitle">Compose a professional message and send it through the configured provider.</p>
                </div>
              </div>
              <p class="subtitle">Provider: Resend</p>
            </div>
          </header>
          <main class="shell">
            {{feedbackHtml}}
    ...
    bool IncludeAttachment)
{
    public static ComposeRequest Default { get; } = new(
        ...
        Subject: "Follow-up from Procyon.Email",
